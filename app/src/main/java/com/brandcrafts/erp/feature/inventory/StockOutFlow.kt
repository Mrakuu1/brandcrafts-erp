...
@Composable fun StockOutScreen(s:StockOutUiState,event:(StockOutUiEvent)->Unit){when{ s.loading->LoadingView(message=stringResource(R.string.stock_out_loading));s.name.isBlank()->ErrorState(stringResource(R.string.stock_out_error_title),stringResource(s.error?:R.string.stock_out_error_material),stringResource(R.string.retry),{event(StockOutUiEvent.Retry)},secondaryActionLabel=stringResource(R.string.cancel),onSecondaryAction={event(StockOutUiEvent.Cancel)});else->InventoryFormSheet(stringResource(R.string.stock_out_title),stringResource(R.string.stock_out_save),{event(StockOutUiEvent.Save)},{event(StockOutUiEvent.Cancel)},primaryActionLoading=s.saving,cancelActionLabel=stringResource(R.string.cancel)){Column(Modifier.fillMaxWidth(),verticalArrangement=Arrangement.spacedBy(12.dp)){s.error?.let{Text(stringResource(it),color=MaterialTheme.colorScheme.error)};InventoryFormTextField(s.name,{},stringResource(R.string.stock_out_material),readOnly=true);InventoryFormTextField(s.quantity,{event(StockOutUiEvent.Quantity(it))},stringResource(R.string.stock_out_quantity),errorMessage=s.quantityError?.let{stringResource(it)},keyboardType=KeyboardType.Decimal);InventoryFormTextField(s.reason,{event(StockOutUiEvent.Reason(it))},stringResource(R.string.stock_out_reason));InventoryFormTextField(s.remarks,{event(StockOutUiEvent.Remarks(it))},stringResource(R.string.stock_out_remarks),singleLine=false)}}}}
